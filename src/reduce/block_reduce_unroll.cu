...
__global__ void BlockUnrollSumReductionKernel(float* input, float* output) {
    ...
    int thread_write_location = 4 * global_thread_id;
    

    // Do first addition between the mapped threads spaced by 2 and the next value
    input_shared[thread_id] = input[thread_write_location] + input[thread_write_location + 1] + input[thread_write_location + 2] + input[thread_write_location + 3];
