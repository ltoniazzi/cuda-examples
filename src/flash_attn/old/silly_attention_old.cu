...
constexpr int n_out_max = 64;
constexpr int o_per_thread_x = n_out_max / 32;
    ...
    for (int i=0; i < T_r; i++) {
        ...
        for (int j = 0; j < T_c; j++){
            ...
            for (int ii = 0; ii < o_per_thread_x; ii++) {
                float m = m_i[ii];
                float last_m = m;
                int row_of_S_i = ii * blockDim.x + tid_x; // must be < B_r
                for (int jj = 0; jj < B_c; jj++) {
                    if (m < S_i[row_of_S_i][jj]) {
                        m = S_i[row_of_S_i][jj];
                    }
                }
                m_i[ii] = m;
                ...
                for (int jj = 0; jj < B_c; jj++) {
                    float S_ij = exp(S_i[row_of_S_i][jj] - m);
                    l += S_ij;
                    for (int dd = 0; dd < o_per_thread_y; dd++) {
...
std::tuple<torch::Tensor, torch::Tensor> silly_attention(torch::Tensor K, torch::Tensor Q, torch::Tensor V) {
    ...
    dim3 tpb(block_x_dim, block_y_dim); // TODO swapped
    dim3 blocks(1); // Not 1 swapped
